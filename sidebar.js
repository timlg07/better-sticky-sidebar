 ...
 * @param {string | null} stickyHeaderSelector The CSS/query-selector for your sticky/fixed header
...
function betterStickySidebar({
    ...
    function limitByViewportTop(force = false) {
        ...
        // We are scrolling above the element => reduce the top space to keep it stuck to the top
    ...
    function limitByViewportBottom() {
        ...
        // => increase top space to keep it stuck to the bottom
        if (belowSidebar) {
            updateSidebarPosition(currentSidebarTop + delta);
        } else {
            // check bounds
